logical function rhyme_irs_iterate_test () result (failed)
  ...
  call irs_iterate_test_cases( irs_Sod_test, "Sod", irs_tester )
  call irs_iterate_test_cases( irs_123_test, "123 test", irs_tester )
  call irs_iterate_test_cases( irs_left_blast_wave_test, "left blast wave", irs_tester )
  call irs_iterate_test_cases( irs_right_blast_wave_test, "right blast wave", irs_tester )
  call irs_iterate_test_cases( irs_two_shocks_collision_test, "two shock collision", irs_tester )

  failed = irs_tester%failed()

contains
  subroutine irs_iterate_test_cases ( func, test_name, tester )
    implicit none
