...
contains
  ...
  module function rhyme_nombre_derived_unit_mul_cc ( c1, c2 ) result ( chain )
    ...
    type ( nombre_derived_unit_t ), pointer :: c1_ptr, c2_ptr
    type ( nombre_derived_unit_t ), pointer :: c1clone_ptr, c2clone_ptr
    ...
    if ( associated( c1_ptr ) ) then
      ...
      do while ( associated( c1_ptr%next ) )
        c1clone_ptr%next => rhyme_nombre_derived_unit_clone( c1_ptr%next )
    ...
    if ( associated( c2_ptr ) ) then
      ...
      do while ( associated( c2_ptr%next ) )
        c2clone_ptr%next => rhyme_nombre_derived_unit_clone( c2_ptr%next )
