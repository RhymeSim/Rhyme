submodule ( rhyme_nombre_units ) mul_smod
contains
  module function rhyme_nombre_units_mul ( u, mul ) result ( new_u_tail )
    ...
    type is ( nombre_unit_t )
      u_tail => rhyme_nombre_units_tail( rhyme_nombre_units_clone( u ) )
      u_tail%next => rhyme_nombre_units_clone( m )
      u_tail%next%prev => u_tail

      new_u_tail => rhyme_nombre_units_tail( u_tail )

    type is ( integer )
      new_u_tail => rhyme_nombre_units_clone( u )
