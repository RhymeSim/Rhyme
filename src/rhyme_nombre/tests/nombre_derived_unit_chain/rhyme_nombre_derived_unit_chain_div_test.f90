logical function rhyme_nombre_derived_unit_chain_div_test () result ( failed )
  ...
  duu => du / meter**2
  call tester%expect( duu == du .toBe. .true. .hint. 'duu w/ symb' )
  ...
  duu => du / meter**2
  call tester%expect( duu%head == kilogram .toBe. .true. .hint. 'duu w/o symb' )
  ...
  ducu => duc / ( kilo * meter )
  call tester%expect( ducu == joule .toBe. .true. .hint. 'ducu w/ symb' )
  ...
  ducu => duc / kelvin
  call tester%expect( ducu == joule .toBe. .true. .hint. 'ducu w/o symb' )
  ...
  uduc => kilogram / duc
  call tester%expect( uduc%prefix == null_prefix .toBe. .true. .hint. 'uduc w/ symb' )
  ...
  duc => nom_duc_factory%generate_chain( [ &
    ...
    year, watt ] )
  uuduc => meter / duc
  call tester%expect( uuduc%prefix == null_prefix .toBe. .true. .hint. 'uuduc w/o symb' )
  ...
  ducduc => duc / duc2
  call tester%expect( ducduc == solar_mass .toBe. .true. .hint. 'ducduc' )
  call tester%expect( ducduc%next == parsec .toBe. .true. )
  call tester%expect( ducduc%next%next == year**(-2) .toBe. .true. )
  call tester%expect( ducduc%next%next%next == hertz**(-1) .toBe. .true. )
  call tester%expect( ducduc%next%next%next%next == radian .toBe. .true. )
