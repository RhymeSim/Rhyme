logical function rhyme_nombre_derived_unit_chain_mul_iduc_test () result ( failed )
  ...
  do i = 1, 1
    ...
    call tester%expect( duc%conv == int( rnd(1) ) * du(1)%conv .toBe. .true. )
    call tester%expect( duc%next%conv == int( rnd(2) ) * du(2)%conv .toBe. .true. )
    call tester%expect( duc%next%next%conv == int( rnd(3) ) * du(3)%conv .toBe. .true. )
    ...
    call tester%expect( duc%conv == int( rnd(1) ) * du(1)%conv .toBe. .true. )
    call tester%expect( duc%next%conv == int( rnd(1) ) * du(2)%conv .toBe. .true. )
    call tester%expect( duc%next%next%conv == int( rnd(1) ) * du(3)%conv .toBe. .true. )
    call tester%expect( associated( duc%next%next%next%next ) .toBe. .false. .hint. 'right end' )
  end do
