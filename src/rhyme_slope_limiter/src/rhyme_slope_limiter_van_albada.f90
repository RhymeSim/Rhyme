submodule(rhyme_slope_limiter) rhyme_sl_van_albada_smod
contains
pure module subroutine rhyme_slope_limiter_van_albada(sl, ul, u, ur, delta)
   ...
   do i = cid%rho, cid%e_tot
      if (r(i) > 0.d0) then
         delta(i) = &
            min(r(i)*(1.d0 + r(i))/(1.d0 + r(i)**2.d0), rhyme_slope_limiter_xi_R(sl, r(i)))*d(i)
      else
         delta(i) = 0.d0
      end if
   end do
end subroutine rhyme_slope_limiter_van_albada
end submodule rhyme_sl_van_albada_smod
