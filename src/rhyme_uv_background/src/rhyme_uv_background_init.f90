...
module subroutine rhyme_uv_background_init(uvb, physics, logger)
   ...
   rho_to_code_unit = 1d0.u. (hydrogen_mass/(centi*meter)**3) .to.physics%rho

   uvb%rho_to_code_unit = rho_to_code_unit%v
   call logger%log('rho code unit conversion', '['//trim(.printchain.physics%rho)//']', '=', [uvb%rho_to_code_unit])
