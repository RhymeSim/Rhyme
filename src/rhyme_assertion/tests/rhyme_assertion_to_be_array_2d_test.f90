logical function rhyme_assertion_to_be_array_2d_test() result(failed)
   use rhyme_assertion

   implicit none

   integer :: int_arr_value(3, 2)
   real(kind=4) :: real_arr_value(3, 2)
   real(kind=8) :: double_arr_value(3, 2)
   character(len=32) :: char_arr_value(3, 2)
   logical :: log_arr_value(3, 2)

   integer :: int_arr(3, 2)
   real(kind=4) :: real_arr(3, 2)
   real(kind=8) :: double_arr(3, 2)
   character(len=32) :: char_arr(3, 2)
   logical :: log_arr(3, 2)

   type(test_t) :: test_i, test_r, test_d, test_c, test_l

   int_arr_value = reshape([1, 2, 3, 4, 5, 6], [3, 2])
   real_arr_value = reshape([1.e0, 2.e0, 3.e0, 4.e0, 5.e0, 6.e0], [3, 2])
   double_arr_value = reshape([1.d0, 2.d0, 3.d0, 4.d0, 5.d0, 6.d0], [3, 2])
   char_arr_value = reshape(['1', '2', '3', '4', '5', '6'], [3, 2])
   log_arr_value = reshape([.false., .true., .false., .true., .true., .false.], [3, 2])
