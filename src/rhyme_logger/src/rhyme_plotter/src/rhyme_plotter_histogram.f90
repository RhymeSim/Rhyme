...
contains
   pure module function rhyme_plotter_histogram_1d( &
      ...
      if (present(minmax)) then
         ...
         hist%min = rhyme_plotter_masked_min(d, msk)
         hist%max = rhyme_plotter_masked_max(d, msk)
      end if
      ...
      case (plid%linear)
         do i = 1, size(d)
            if (rhyme_plotter_is_masked(d(i), msk)) then
               hist%n_masked = hist%n_masked + 1
               cycle
            end if
      ...
      case (plid%linear)
         do j = lbound(d, dim=2), ubound(d, dim=2)
            do i = lbound(d, dim=1), ubound(d, dim=1)
               ...
               if (rhyme_plotter_is_masked(d(i, j), msk)) then
                  hist%n_masked = hist%n_masked + 1
                  cycle
               end if
         ...
         do k = lbound(d, dim=3), ubound(d, dim=3)
            do j = lbound(d, dim=2), ubound(d, dim=2)
               do i = lbound(d, dim=1), ubound(d, dim=1)
                  ...
                  if (rhyme_plotter_is_masked(d(i, j, k), msk)) then
                     hist%n_masked = hist%n_masked + 1
                     cycle
                  end if
