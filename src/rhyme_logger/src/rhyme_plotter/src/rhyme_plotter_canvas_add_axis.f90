  ...
  contains
    ...
    subroutine rhyme_plotter_canvas_add_vertical_axis ( axis, color )
      ...
      if ( axis .eq. plid%left ) then
        ...
        if ( present( color ) ) then
          write( tick_char, '(A12,A1,A4)' ) color, char( int( z'2524' ), ucs4 ), colors%nc
        else
        ...
        if ( present( color ) ) then
          write( tick_char, '(A12,A1,A4)' ) color, char( int( z'251C' ), ucs4 ), colors%nc
        else
      ...
      if ( present( color ) ) then
        write( axis_char, '(A12,A1,A4)' ) color, char( int( z'2502' ), ucs4 ), colors%nc
      else
      ...
      do y = 1, canvas%y
        if ( y .eq. int( nbin * canvas%axes(axis)%tick_width_px ) + 1 .or. y .eq. canvas%y ) then
          ...
          do x = label_start, label_end
            canvas%table( x, canvas%y - y + 1 ) = canvas%axes(axis)%tick_labels(nbin)( x-label_start+1:x-label_start+1 )
          end do
    ...
    subroutine rhyme_plotter_canvas_add_horizontal_axis ( axis, color )
      ...
      if ( axis .eq. plid%bottom ) then
        ...
        if ( present( color ) ) then
          write( tick_char, '(A12,A1,A4)' ) color, char( int( z'252C' ), ucs4 ), colors%nc
        else
        ...
        if ( present( color ) ) then
          write( tick_char, '(A12,A1,A4)' ) color, char( int( z'2534' ), ucs4 ), colors%nc
        else
      ...
      if ( present( color ) ) then
        write( axis_char, '(A12,A1,A4)' ) color, char( int( z'2500' ), ucs4 ), colors%nc
      else
      ...
      do x = 1, canvas%x
        if ( x .eq. int( nbin * canvas%axes(axis)%tick_width_px ) + 1 .or. x .eq. canvas%x ) then
          ...
          do xl = label_start, label_start + label_len - 1
            canvas%table( xl, labels_row ) = canvas%axes(axis)%tick_labels(nbin)( xl-label_start+1:xl-label_start+1 )
          end do
        end if
      end do
